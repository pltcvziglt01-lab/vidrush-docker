...
|---|---|---|---|
| 15 Ağu | **Y-11b-2 son iki P0: paralel yolda bayrak kaybı + tek yönlü süre kapısı** | _(bu commit)_ | ⚠ **`Y11B2-PARALEL-BAYRAK-KAYBI`** — `uzun_plan` 55'ten fazla sahnede **paralel** yola giriyor ve `havuz.submit(plan_uret, ...)` **pozisyonel** çağrıldığı için `grounded_exact` hiç geçmiyordu; aynı kayıp `_uzun_plan_sirali` iskelet-fallback'inde ve paralel yolun final `sure_tamamla` çağrısında da vardı. Sonuç: uzun belgesellerde exact sözleşme **sessizce düşüyor**, planlayıcı kelime bandına dönüyor ve exact kapı **zorunlu RED** veriyordu → bayrak üç noktada da **keyword** ile zincirlendi; red-first 60-sahne paralel + iskelet-fallback **spy** testleri her parçanın `grounded_exact=True` aldığını ölçüyor. ⚠ **`Y11B2-SURE-TEK-YONLU`** — grounded süre kapısı yalnızca **alt** sınırı ölçüyordu: 1 dk / 150 wpm hedefinde bütçe 150 kelimeyken **300** ve **600** kelimelik planlar da PASS alıyordu (video hedefin 2–4 katı uzun). Kanonik fact metni yeniden yazılamadığı için fazlalık **kırpılamaz** → deterministik **iki yönlü** bant (`GROUNDED_SURE_ALT=0.92`, `GROUNDED_SURE_UST=1.15`), bant dışında medya/TTS/render'dan **önce** `GROUNDED-SURE-YETERSIZ`. Grounded olmayan modlar **etkilenmez**. **Koşum**: `test_faz_y11b2` **229/0**, `y11b1` **168/0**, `y11b` **91/0**, `y11` **46/0**, `y10` **26/0**, `a` **125/0**, `y18` **83/0**, `test_faz_i` **2842/0**; `git diff --check` temiz. Mac'te medya/render **üretilmedi**, $0.00 |
| 15 Ağu | **Y-11b-2 konuşulan-alan çözümleyicisi — iki kapatılan P0** | _(commit BEKLİYOR — denetim; koşum sayıları denetim sonrası yazılacak)_ | ⚠ **`Y11B2-VOICEOVER-KOR`** — `sahne_metni()` üretim sahnelerinin anlatımı taşıdığı **`voiceover`** alanını okumuyordu (`pipeline.py`: `metin = str(s.get("voiceover", ""))`). Entailment bu yüzden **boş metin** ölçüyor, her sahne ilgisiz sayılıyor ve grounded belgesel **her zaman** duruyordu. Kusur, 0-call ölçümünün boş olup olmadığını sınayan **pozitif kontrol koşumunda** ortaya çıktı: kapı geçildiğinde downstream spy'ları hiç ateşlenmiyordu. ⚠ **`Y11B2-YETKI-AKLAMA`** — aynı çözümleyici konuşulan metni yardımcı alanlarla (`iddia_metni`, `footage_sorgu`, `scene_prompt`, `gorsel_prompt`) **birleştiriyordu**. `iddia_metni` zaten fact'in kendi metni olduğu için, izleyicinin **duyduğu** cümle fact'le tamamen ilgisiz olsa bile skor yardımcı alanlardan geliyor ve tahsis PASS alıyordu — kimlik **aklanıyordu**. **Ortak çözüm**: ikisi de tek bir fail-closed çözümleyiciye (`fact_baglama.konusulan_alan`) bağlandı — yetkili konuşulan alan **yalnızca `voiceover`**, boş olmayan gerçek bir dizge olmak zorunda; yardımcı alanlar skoru **artıramaz**, `anlatim`/`narration` fallback'i **kaldırıldı** ve TTS **aynı** çözümleyicinin çıktısını tüketir (`Y11B2-RESOLVER-AYRISMASI`). Mac'te medya/render **üretilmedi**, deploy YOK, $0.00 |
| 15 Ağu | **Y-11b-2 grounded fact HAT ENTEGRASYONU: tahsis otoritesi benzerlik değil ENTAILMENT** | _(commit BEKLİYOR — denetim)_ | ⚠ **ÖLÇÜLEN KUSURLAR (hepsi davranışsal kanıtlandı)**: **`Y11B2-BENZERLIK-OTORITESI`** — üretim yolu `fact_bagla` idi ve tahsisi **0.16 Jaccard** ile yapıyordu; kanıt: allowlist'te **olmayan** `UYDURMA-KIMLIK-000` bir footage sahnesine YAZILDI (`baglanan=1`). **`-KAPI-CAGRILMIYOR`** — `grounded_kapisi` + `shot_fact_dogrula` tanımlıydı ama `pipeline.py` ikisini de HİÇ çağırmıyordu (**ölü kod**). **`-KOSULSUZ-DEGIL`** — tek fail-closed duruş `if _olgular:` bloğunun İÇİNDEYDİ. **`-ENTAIL-POLARITE-KOR`** — fact "recorded" iken anlatım "**never** recorded" PASS alıyordu. **`-ILK-SAHNE-CLOSING`** — chapter'ın ilk sahnesi closing ise yepyeni fact alıyordu. **`-BOS-SHOT-PASS`** — `shot_fact_dogrula([])` PASS sayılıyordu. **`-SHOT-RAPORU-YOK`** — kapı tüm-shot bilgisi olmadan PASS verebiliyordu. **`-ENV-KAPI-GEVSETME`** — `FACT_KAPSAM_ESIGI=0` kapıyı tamamen kapatıyordu. **`-PROJEKSIYON-YOK`** — üretim sahneleri `bolum`/`islev` taşır, `tahsis_et` `chapter_id`/`beat_role` bekler. ⚠ **`Y11B2-VOICEOVER-KOR`** — **pozitif kontrol koşumunun ortaya çıkardığı kusur**: üretim sahneleri anlatımı `voiceover` alanında taşır (`pipeline.py:4790`); `sahne_metni()` bu alanı okumuyordu, yani entailment **boş metin** ölçüyor ve grounded belgesel HER ZAMAN duruyordu. **Çözüm**: tahsis `allowlist + entail_dogrula` (deterministik, eşitlikte alfabetik); `allowlist` **zorunlu**; prefilled kimlik allowlist'te olmalı VE entail etmeli; kapı GROUNDED modda **koşulsuz** ve medya/TTS/render'dan ÖNCE; eşik `max(FACT_KAPSAM_TABANI=1.0, env)` ile **kilitli**. ⚠ **0-CALL ÖLÇÜMÜ BOŞ DEĞİL**: suite bir **POZİTİF KONTROL** taşır — kapı geçildiğinde spy'ların gerçekten ateşlendiği doğrulanır (ölçülen: `tts=3`); ateşlenmezse test RED verir. **Koşum**: `test_faz_y11b2` **119/0** (YENİ), `y11b1` **168/0**, `y11b` **82/0**, `y11` **46/0**, `y10` **26/0**, `a` **125/0**, `y18` **83/0**; tam A–I yeşil (`test_faz_i` **2842/0**, b 200/0, c 152/0, d 95/0, e 127/0, f 247/0, g 219/0, h 280/0). ⚠ Grounded OLMAYAN modlar (animasyon/hikaye) **kapsam dışı** — davranış bit-bit aynı. ⚠ **FINAL DENETİMİN 6 EK P0'I** — hepsi red-first kapatıldı: **`Y11B2-DURAGAN-PROJEKSIYON`** — projeksiyon `bolum or "c01"` biçiminde **duraağandı**; üretim planında `bolum` başlığı yalnızca bölümün ilk sahnesinde dolar, araya girenler boş gelir ve hepsi `c01`'e yığılıyordu; ayrıca rol `islev`'in **kendisi** (`kapanis`) yazılıyor, kanonik `closing` görülmüyordu → ortak `fact_baglama.yay_projeksiyonu()` (canlı bölüm sayacı + `ISLEV_YAY_ROLU`), `yay_plani_kur` de **aynı** helper'ı kullanır (kopya mantık yok). **`-FACT-ID-FALLBACK`** — `primary_fact_id or fact_id` fallback'i birincil alanı boş shot'u geçiriyordu → **iki alan da zorunlu ve birebir eşit**. **`-GLOBAL-POLARITE-KOR`** — tek global olumsuzluk boolean'ı yetmiyordu: "did **not** increase mortality **but** reduced hospitalization" vs ters dağılım her ikisinde de olumsuzluk taşıdığı için PASS alıyordu → `polarite_dagilimi()` + `polarite_carpisiyor_mu()` ile **yüklem bazında** ölçüm. **`-GEC-PREFLIGHT`** — kapı `uzun_plan`'dan **sonra** koşuyordu → **ön-kapı** araştırma döner dönmez; `uzun_plan`/`metin_islev_analizi`/`os.makedirs` dahil **0-call**. **`-SONUC-ALAN-YOK`** — kapı `hata` ve `cozulemeyen` alanlarını okuyordu ama `Sonuc` bunları **taşımıyordu**, yani iki dal üretimde **hiç tetiklenemezdi** → alanlar eklendi, çökme dalları `hata_yaz()` ile doldurur. **`-VOICEOVER-KOR`** — üretim sahneleri anlatımı `voiceover` alanında taşır; `sahne_metni()` okumuyordu, entailment **boş metin** ölçüyor ve grounded belgesel her zaman duruyordu. ⚠ **TEST HERMETİK**: `footage_getir`, `youtube_sahne`, `atif_al`, `magnific_upscale`, `stok_provenans_al`, `medya_kopru.sahne_medyasi`, `referansli_gorsel`, `uretmod.seslendir`, `subprocess.run/Popen` sınırlarının **hepsi** stub'lı, `try/finally` ile geri konur; ağ/kimlik bilgisi/medya yok. Sahte ikiz yerine **gerçek `arastirma_kopru.Sonuc`** kullanılır. Yaratıcı modlar (animasyon/hikaye) tam hat regresyonuyla doğrulanır: grounded helper'lar **çağrılmaz**, hat downstream sınıra **ulaşır**. ⚠ **FİNAL DENETİMİN 2 EK P0'I**: **`Y11B2-YETKI-AKLAMA`** — `sahne_metni()` konuşulan metni yardımcı alanlarla (`iddia_metni`, `footage_sorgu`, `scene_prompt`, `gorsel_prompt`) **birleştiriyordu**; `iddia_metni` zaten fact'in kendi metni olduğu için izleyicinin **duyduğu** cümle fact'le tamamen ilgisiz olsa bile skor yardımcı alanlardan geliyor ve tahsis PASS alıyordu — kimlik **aklanıyordu** → entailment otoritesi yalnızca **ilk dolu konuşulan alan** (`voiceover` > `anlatim` > `narration`); yardımcı alanlar skoru **artıramaz** (tek başlarına `""` döner). **`Y11B2-KANONIK-BYPASS`** — iki kaçak ölçüldü: (a) `beat_role="kapanis"` normalize edilmediği için `SONUC_ROLLERI` ile eşleşmiyor ve **kapanış yasağını atlıyordu**; (b) açıkça verilmiş `chapter_id="c02"` ham sayaç tarafından `c01`'e **indiriliyor**, ikinci bölümün kapanışı birinci bölümün fact'lerini "önceki" sayıyordu → `rol_normalize()` ile kanonik indirgeme, kanonik `chapter_id` **korunur** (sayaç onunla senkronlanır), ham `bolum`/`islev` durum taşıyan sayaçla eşlenir, ikisi aynı sahnede **karışırsa** `SHOT-PROJEKSIYON-BELIRSIZ` (fail-closed). Pipeline sahneye kanonik alan **yazmaz** — aksi halde her sahne kendi kendine "karışık" olurdu; projeksiyon `tahsis_et`/`yay_plani_kur` içinde **ortak** helper'la çözülür. ⚠ **EK FİNAL POLARİTE P0'LARI**: **`Y11B2-BAGLAC-BOLUNMEDI`** — ayırıcı kümesinde `and`/`also` ve **cümle sonu** yoktu; ölçülen kaçak: fact "recorded cases **and did not** disclose names" vs sahne "**did not** record cases **and** disclosed names" → olumsuzluk iki yüklem arasında yer değiştirmişken tek yantümce görülüyor ve kapı **PASS** veriyordu. Nokta **yalnızca cümle sonunda** ayırıcıdır — `39.4%` ve `76.941` bölünmez. **`Y11B2-KAYNAK-TEKRARI`** — `onerme` + `exact_quote` koşulsuz birleştiriliyordu; TAM EŞİTLİK sözleşmesi gereği ikisi normalde **aynı** olduğu için aynı cümle iki kez yazılıyor ve sınırda **sahte** bir birleşik yantümce oluşuyordu. **`Y11B2-COKLU-ESLESME`** — eşleşme her fact yantümcesi için bağımsız "en iyi"yi seçiyordu; o sahte yantümce yüzünden **birebir aynı** metin bile `FACT-ENTAIL-POLARITE` **RED** alıyordu → eşleşme artık **bire-bir** (en yüksek puanlı çiftler önce, her yantümce en çok bir kez; eşitlikte düşük indeks — deterministik). `and`/`also`/virgüllü üç varyantta ters dağılım RED, birebir aynı metin PASS. Mac'te medya/render **üretilmedi**, deploy YOK, $0.00 |
| 15 Ağu | **Y-11b-1c bağlam kapısı: sabit pencere değil KAPSAYAN İDDİA BLOĞU** | _(bu commit)_ | ✅ `558ad71` sonrası bağımsız denetim **4 in-scope bağlam varyantını FAIL-OPEN** yakaladı; dördü de red-first kapatıldı. ⚠ **`Y11B1-SPAN-IDDIA-DEGIL`** — span'ın **kendisi** soru cümlesi olabiliyordu ("Did the agency record 76,941 cases in 2024?"): TAM EŞİTLİK sözleşmesini geçiyor ama **hiçbir şey önermiyordu** → `iddia_mi()` hem `onerme` hem `exact_quote` için `?` taşıyanı reddeder (`FACT-SPAN-IDDIA-DEGIL`). ⚠ **`Y11B1-SABIT-PENCERE-KOR`** — ±160 karakterlik **sabit** pencere, aynı paragrafta quote'tan **160+ karakter uzakta** duran "this myth was debunked" cümlesini **göremiyordu**; ayrıca `FALSE: <quote>` ve "Officials **rejected the claim that** <quote>" kalıpları cue listesinde yoktu → çevre artık karakter sayısıyla değil **quote'u kapsayan iddia bloğu** (`kapsayan_blok()`, paragraf temelli; blok çözülemezse fail-closed **tüm belge**) ile tanımlanır ve cue seti `false/verdict/debunk/reject/claim that/alleged/untrue/fake/baseless/reddetti/iddiasi…` ile genişletildi (`FACT-SPAN-CEVRESI-CURUTUYOR`). Quote'u taşıyan **cümle** `?` ile bitiyorsa da RED. Cue taraması quote'un **kendi metni hariç** yapılır (kendi kendini RED etmez). **Mevcut canonical true assertion HÂLÂ PASS** — kapı gevşetilmedi, daraltıldı. **Koşum**: `test_faz_y11b1` **168/0** (+17), `test_faz_y11b` **80/0**, `test_faz_y11` **46/0**, `test_faz_y10` **20/0**, `test_faz_a` **125/0**; tam A–I yeşil (`test_faz_i` **2841/0**, b 200/0, c 152/0, d 95/0, e 127/0, f 247/0, g 219/0, h 280/0); y12 119/0, y13b 66/0, y16 47/0, y17 89/0, y18 83/0; `git diff --check` temiz. ⚠ **KAPSAM**: yalnız bağlam kapısı — hat entegrasyonu (`fact_bagla` Jaccard, `grounded_kapisi`, tahsis) **Y-11b-2'de AÇIK**. Mac'te medya/render **üretilmedi**, deploy YOK, $0.00 |
| 15 Ağu | **Y-11b-1 araştırma gerçek FactPacket üretiyor; kanıt YENİDEN DOĞRULANARAK otorite olur** | _(bu commit)_ | ✅ **son gerçek koşum**: A–I tam yeşil — 38 paket **0 hata**, `test_faz_i` **2841/0**; `test_faz_y11b1` **151/0**, `test_faz_y11b` **80/0**, `test_faz_y11` **46/0**, `test_faz_y10` **20/0**, `test_faz_a` **125/0**; `git diff --check` temiz. ⚠ **`Y11B1-PAKET-SONUCA-TASINMIYOR`**: `factpacket.havuz_kur` KANIT-ÖNCE paket üretiyordu ama **hiçbir yerden çağrılmıyordu**; hat `Sonuc.olgular`'ı hâlâ CLAIM-FIRST zincirinden türetiyordu. **Çözüm**: `paket_havuzu_kur()` → FactPacket → `allowlist_kur`. ⚠ **BAĞIMSIZ DENETİM 10 TUR — hepsi red-first kapatıldı**; son tur tavşan deliğini kesti: (9) **`Y11B1-SIGNER-ORACLE`** — doğrulama çıktısını (public snapshot **veya** HMAC mührü) otorite saymak imzalayıcıyı **oracle** yapıyordu → otorite artık bir damga değil, **tüketicinin `paketler` + `replay_belgeleri` ile YENİDEN KOŞTUĞU doğrulama**; `Sonuc.olgular` yalnızca görünüm. **`-RAW-ATIF`** — pipeline atıfları ham `olgular` dict'lerinden üretiyordu → `yetkili_olgular()` ile yeniden doğrulanmış otoriteden. **`-SOURCE-BAGI-YOK`** — `source_id` ile `url` bağımsızdı, replay sonrası evil URL sızabiliyordu → `source_id == source_id_uret(canonical_url)` **zorunlu**. **`-ERKEN-CELISKI-KARARI`** — çelişki kararı iki yerde veriliyordu → tek otorite `allowlist_kur`. **`-EXTRACTIVE-YETMEZ`** — "önerme quote'un bir parçası" kuralı quote **bağlamını** kaybettiriyordu ("It is false that X", "Officials denied X", söylenti/soru cümleleri) → **EN DAR MVP: önerme, normalize `exact_quote` ile TAM EŞİT** (alt-dizgi/paraphrase kabul edilmez). **`-REFUTE-NLI-YOK`** — çelişki kararını token/pattern ile vermek her turda yeni kaçak üretti → `refute` paketleri Y-11b-1'de **fail-closed `unresolved`**: allowlist'e girmez, çelişki kurmaz, **geçerli support'u zehirleyemez**; gerçek karar dedicated NLI ile gelecek. Zaman kapsamı tam **ISO** (`YYYY-MM-DD`>`YYYY-MM`>`YYYY`). (10) **`Y11B1-KIMLIK-YENIDEN-TURETIM`** — tüketici replay'i üreticiden ZAYIFTI: `fact_id` paketin **beyan ettiği** haliyle taşınıyordu, böylece **eski ID altında mutasyona uğramış claim** geçebiliyordu → `allowlist_kur` replay'den ÖNCE `kimlik_icerikten_mi(p)` ile `fact_id`'yi **güncel içerikten yeniden türetir**; uyuşmazlık `FACT-KIMLIK-ICERIKTEN-TUREMIYOR`. **`Y11B1-SPAN-CEVRE-BAGLAMI`** — TAM EŞİTLİK bile span'ın **çevresini** görmüyordu: "It is false that <quote>", "<quote>. VERDICT: FALSE", "This myth was debunked: <quote>" ve **soru cümleleri** geçiyordu → `cevre_curutuyor_mu()` ±160 karakterde assertion-negation cue'larını (false/myth/rumor/denied/debunked/retracted/disputed/hoax/yalan/asılsız/çürütüldü…) ve `?` işaretini tarar; `FACT-SPAN-CEVRESI-CURUTUYOR`. **`Y11B1-IC-API`** — `paketlerden_olgular` dış `allowlist_sonucu`/`muhurlu` girişleri **kaldırıldı**; otorite yalnız güvenilir getirme makbuzu taşıyan `arastir_ve_zenginlestir` **iç** yolundan gelir. ⚠ **TEHDİT MODELİ AÇIK SINIRI**: in-process **kötü niyetli çağıran** (sahte `belgeler` sözlüğü uyduran kod) bu atomun tehdit modeli **DIŞINDADIR** — kapı, dış kaynak/ağ verisinin sessizce olguya dönüşmesini engeller, aynı süreçte çalışan düşman kodu değil. Önceki 8 turun kodları korunuyor (`-REPLAY-BELGESIZ-KABUL`, `-MUTABLE-STATUS-GUVENI`, `-PLANNER-ESKI-CLAIM`, `-IKI-OTORITE`, `-ONERME-QUOTE-UYUMSUZ`, `-UYUM-IMPORT-FAIL-OPEN`, `-KATMAN-KOD-SOZLESMESI`, `-IKINCI-OTORITE`, `-HAM-DICT-OTORITE`, `-STALE-REFUTE-ZEHIRI`, `-CELISKI-ZAMAN-KOR`, `-ZAMAN-KAPSAMI-KABA`, `-MUHURSUZ-ALAN-MUTASYONU`). `researcher` genel ilk-2 atıf fallback'i **kaldırıldı**. ⚠ **KAPSAM SINIRI**: bu commit **hat entegrasyonu İÇERMEZ** — `fact_bagla` Jaccard(0.16) **hâlâ canlı**, `grounded_kapisi`/tüm-shot denetimi ve pipeline tahsis/shot entegrasyonu **Y-11b-2 kapsamında AÇIK**. `pipeline.py`'de tek değişiklik atıf çağrısının yeniden doğrulanmış otoriteye bağlanması (**12 satır**); `test_faz_y10` kapı metni **değişmedi**. Mac'te medya/render **üretilmedi**, deploy YOK, $0.00 |
