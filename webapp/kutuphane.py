...
def kabul_edilebilir_mi(kayit: dict) -> dict:
    ...
    if isinstance(ep, dict) and ep:
        # ⚠ FAZ Y-4 (kullanici duzeltmesi, BAGLAYICI): `render_edilebilir`
        # False ise is KABUL EDILMEZ — NEDEN NE OLURSA OLSUN. Kapi FAIL kod
        # listesine BAKMAZ.
        # ⚠ Y-3'TE YAPILAN HATA GERI ALINDI: teslim kapisi FAIL kodlarini bir
        # "yayin kapilari" listesine gore suzuyordu ve liste disi FAIL'ler
        # (ozellikle `FACT-BAGLANTI-YOK`) teslimi engellemiyordu. Bu,
        # GAZETECILIK IZLENEBILIRLIGINI fiilen gevsetiyordu: "her bolum
        # aciklaniyor olmali" talebi, her anlatici cumlesi/cekimin
        # DOGRULANMIS bir fact_id ve kaynak/provenance ile bagli olmasini
        # ZORUNLU kilar.
        if not ep.get("render_edilebilir"):
            _qa = ep.get("qa") if isinstance(ep.get("qa"), dict) else {}
            _kodlar = sorted({
                _s(x.get("kod")) for x in (_qa.get("sorunlar") or [])
                if isinstance(x, dict)
                and _s(x.get("seviye")).lower() == "fail" and _s(x.get("kod"))
            })
            _nd = ("+".join(_kodlar) if _kodlar
                   else (_s(ep.get("neden")) or _s(_qa.get("durum")).upper()
                         or "render_edilebilir=False"))
