#!/usr/bin/env python3
"""FAZ I-16 — ALTYAZI + GORUNUR KAYNAK KUNYESI + 1080p PREMIUM RENDER.

I-15 kapiyi PASS ettirdi ama uc sey EKSIKTI: altyazi yoktu, kaynak kunyesi
ekranda gorunmuyordu ve cikti yalnizca 1280x720'ydi. Bu betik ucunu de
kapatir ve `kalite_kapisi=True` ile 15-20 sn'lik tarih/belgesel ornegini
1080p olarak yeniden uretir.

I-15'e gore DEGISEN UC SEY:

  1. ALTYAZI — `altyazi` alani sozlesmede VARDI ve adapter onu props'a
     TASIYORDU, ama hicbir Remotion bileseni onu CIZMIYORDU ("props'ta var,
     videoda yok"). Yeni `Altyazi` bileseni eklendi; kupler edge-tts
     `SentenceBoundary` zamanlamasindan uretiliyor ve KELIME SINIRINDA
     satirlara bolunuyor.
     ⚠ DURUST ETIKET: cumle ICI bolunme gerektiginde parca zamanlamasi
     OLCUM DEGIL, karakter agirlikli ORANTILI dagitimdir. edge-tts 7.2.8
     yalnizca cumle siniri veriyor (olculdu), kelime siniri vermiyor.
     Her kup `zamanlama: olculdu|orantili` alani tasir.

  2. KAYNAK KUNYESI — fixture `atif_gerekli: False` dedigi icin kunye HIC
     uretilmiyordu. Artik uretiliyor VE `KaynakEtiketi` bileseninin
     `bottom: 22` sabiti kaldirildi: kunye Python'un hesapladigi konumu
     okuyor ve yayin guvenli kenarini (64 px) zorluyor. 1080p'de eski hali
     guvenli alanin DISINDA kaliyordu.

  3. 1080p — render 1920x1080. Baslik siniri de gercek genislikten
     hesaplandigi icin 1280'de 20 karakter olan sinir burada 31 oluyor.

⚠ HAREKETLI VIDEO B-ROLL: mevcut guvenli fixture havuzunda GERCEK bir
video adayi YOK. Depodaki `.mp4` dosyalarinin hepsi bu projenin KENDI eski
render ciktilaridir (Faz D/E pilotlari) — onlari "arsiv B-roll" diye
kullanmak dongusel ve yaniltici olurdu. Bu yuzden B-roll bacagi dursutce
BLOKE yazilir; sahte hareket URETILMEZ.
...
Cikti:
    outputs/sample/editorv2_altyazi_1080p_i16.mp4 (+ 9 kare ve JSON rapor)
"""
...
os.environ.setdefault("VIDRUSH_KOK", os.path.join(DEPO, "cikti", "_i16_kok"))

CIKTI_DIZIN = os.path.join(DEPO, "outputs", "sample")
VIDEO_ADI = "editorv2_altyazi_1080p_i16.mp4"
FIXTURE = os.path.join(DEPO, "app", "render-studio", "public", "editorv2",
                       "faz_e")
CALISMA = os.path.join(DEPO, "cikti", "_i16")
...
SAHNE_METINLERI = [
    ...
    ("f001", "The lunar module touched down on the Moon on the twentieth "
             "of July, nineteen sixty-nine."),
    ("f002", "During the descent, the guidance computer raised a "
             "twelve-oh-two program alarm."),
    ("f004", "Armstrong took manual control and steered past a field "
             "of boulders."),
]
...
def video_broll_ara():
    """Guvenli fixture havuzunda GERCEK hareketli video adayi var mi?

    ⚠ Depodaki `.mp4` dosyalari bu projenin KENDI render ciktilaridir
    (Faz D/E pilotlari). Onlari "arsiv B-roll" diye kullanmak dongusel
    olurdu: kendi ciktimizi yeniden render edip "gercek cekim" demek.
    Bu yuzden yalnizca fixture MEDYA dizinleri taranir; bulunamazsa
    BLOKE yazilir ve sahte hareket URETILMEZ.
    """
    aday, taranan = [], []
    for dizin in (FIXTURE,
    ...
    return {
        "aday": aday,
        "taranan_dizin": taranan,
        "durum": "VAR" if aday else "BLOKE",
        "sebep": ("" if aday else
                  "guvenli fixture havuzunda hareketli video adayi YOK; "
                  "depodaki .mp4 dosyalari bu projenin kendi render "
                  "ciktilari (Faz D/E pilotlari) ve B-roll DEGILDIR"),
    }
...
def girdi_kur(secilen, sinirlar, kesim_sn):
    ...
    for i in range(n):
        ...
        adaylar.append({
            ...
            # ⚠ I-16: I-15'te bu alan False'ti ve kaynak kunyesi HIC
            # uretilmiyordu (plan `atif_gerekli` sarti ariyor). Public domain
            # olsa da NASA kunyesi EKRANDA gorunmeli.
            "atif_gerekli": True, "baslik": "Apollo 11 archive photograph",
...
def main() -> int:
    print("=" * 72)
    print("FAZ I-16 — ALTYAZI + KAYNAK KUNYESI + 1080p (kapi ACIK)")
    print("=" * 72)
    ...
    for t in kare_anlari:
        ad = f"i16_kare_{str(t).replace('.', '_')}s.png"
    ...
    with open(os.path.join(CIKTI_DIZIN, "altyazi_1080p_i16_rapor.json"), "w",
              encoding="utf-8") as f:
        json.dump(rapor, f, ensure_ascii=False, indent=2)
    print(f"\n      rapor : outputs/sample/altyazi_1080p_i16_rapor.json")
