#!/usr/bin/env python3
"""FAZ I-18 — IKINCI KONSEPT PILOTU: DOGA / SEYAHAT, GERCEK WEB MEDYASI.

I-13'ten I-17'ye kadar HER pilot ayni Apollo fixture'iyla kosuldu. Bu betik
IKINCI bir konsepti — doga/seyahat — uctan uca kosar ve uc seyi kanitlar:

  1. KULLANICI YALNIZ METIN VERIR. Konu metni `taksonomi.siniflandir` ile
     otomatik siniflanir ve `stil_profili.coz` bilesik stili SECER. Tur/stil
     ELLE VERILMEZ; secim `kaynak: "auto"` olarak raporlanir.
     ⚠ I-18'de OLCULEN BOSLUK: taksonominin `seyahat.doga_manzara` dali
     yalnizca HIZMET sozluguyle ("gezi", "rehber", "tur") tetikleniyordu;
     uc ayri doga metni de `belirsiz` cikiyor ve stil `belgesel-sinematik`
     VARSAYILANINA dusuyordu. Manzara/yer sekli sozlugu eklendi (690 -> 783
     anahtar) ve ucu de `seyahat` / kesin oldu.

  2. MEDYA GERCEKTEN EDINILIR. Havuzda tek bir doga gorseli yoktu (olculdu:
     `cikti/faz_e/medya/` 12 varligin 12'si Apollo, `cikti/faz_d/zemin/`
     ise SENTETIK GRADYAN). Bu yuzden `medya.commons` ile Wikimedia
     Commons'tan ANAHTARSIZ ve UCRETSIZ edinilir; lisans karari
     `medya.lisans`, indirme `medya.indirme.guvenli_indir` (SSRF duvari).
     Eser sahibi okunamayan aday ELENIR.

  3. KAYNAK KUNYESI SAHNEYE OZGU. Her sahne KENDI varliginin gercek
     yazar+lisansini gosterir (I-16/I-17'de her sahnede ayni genel etiket
     vardi).

⚠ APOLLO'YA OZEL HICBIR HARD-CODE YOK. Konu, sahne cumleleri ve arama
sorgulari bu dosyanin USTUNDEKI sabitlerden gelir; motor tarafinda konu adi
gomulu degildir.

⚠ 4K IDDIASI KAYNAGA BAGLI. Cikti ancak edinilen kaynaklarin HEPSI
`commons.DORT_K_EN_AZ_GENISLIK` esigini geciyorsa 3840x2160 render edilir;
gecmezse DURUSTCE 1080p'ye duser ve sebep raporlanir. Yukseltme (upscale)
YAPILMAZ — sahte cozunurluk uretilmez.

⚠ HAREKETLI VIDEO B-ROLL: hala BLOKE. Bu atom yalnizca FOTOGRAF edinir.
...
⚠ MALIYET $0.00 — edge-tts ve Commons API anahtar istemez.

Kosum:
    python3 webapp/testler/smoke_konsept2_doga_i18.py
Cikti:
    outputs/sample/editorv2_doga_i18.mp4 (+ 9+ kare ve JSON rapor)
"""
...
os.environ.setdefault("VIDRUSH_KOK", os.path.join(DEPO, "cikti", "_i18_kok"))

CIKTI_DIZIN = os.path.join(DEPO, "outputs", "sample")
VIDEO_ADI = "editorv2_doga_i18.mp4"
FIXTURE = os.path.join(DEPO, "app", "render-studio", "public", "editorv2",
                       "faz_e")
CALISMA = os.path.join(DEPO, "cikti", "_i18")
...
MEDYA_ONBELLEK = os.path.join(DEPO, "cikti", "_i18_medya")
...
def main() -> int:
    print("=" * 72)
    print("FAZ I-18 — IKINCI KONSEPT: DOGA/SEYAHAT + GERCEK WEB MEDYASI")
    print("=" * 72)
    ...
    if eksik:
        ...
        with open(os.path.join(CIKTI_DIZIN, "doga_i18_bloke_rapor.json"),
        ...
        print("      rapor: outputs/sample/doga_i18_bloke_rapor.json")
        return 2
    ...
    for t in kare_anlari:
        ad = f"i18_kare_{str(t).replace('.', '_')}s.png"
    ...
    with open(os.path.join(CIKTI_DIZIN, "doga_i18_rapor.json"), "w",
              encoding="utf-8") as f:
        json.dump(rapor, f, ensure_ascii=False, indent=2)
    print(f"\n      rapor : outputs/sample/doga_i18_rapor.json")
