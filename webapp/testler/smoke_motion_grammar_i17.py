#!/usr/bin/env python3
"""FAZ I-17 — BELGESEL MOTION GRAMMAR + OPTIK DURAGANLIK KAPISI.

I-16 teknik olarak temizdi (1080p, altyazili, kunyeli, PRE/POST QA PASS) ama
IZLEYICI ICIN amatordu: dort statik fotograf 17.6 sn boyunca duruyordu ve bir
sahne neredeyse HIC DEGISMIYORDU.

ONCE DURUMU — I-16 ciktisinda OLCULDU (4 fps / 64x36 gri, ardisik ortalama
mutlak fark):

    b001 push-in    2.96 sn   optik ortalama 3.551
    b002 static     5.21 sn   optik ortalama 0.914   <-- DURAGANLIK KUSURU
    b003 push-in    4.69 sn   optik ortalama 5.102
    b004 pull-out   4.68 sn   optik ortalama 7.030
    gecis: 4/4 hard-cut  ·  hareket: push-in IKI KEZ (b001, b003)

Bu betik ayni Apollo fixture'iyla, ayni guvenli kod kapsaminda:
  1. `static`i uzun cekimden CIKARIR (Ken Burns yonu atanir)
  2. hareket tekrarini ARDISIK OLMAYAN pencerede de engeller
  3. acilis push-in / kapanis pull-out ritmi uygular
  4. kapanis beat'ine ISLEVE BAGLI `karartma` gecisi getirir (ikinci aile)
ve ciktiyi AYNI olcumle yeniden dogrular.

⚠ ESIKLER GERCEK ONCE/SONRA OLCUMUNDEN turetildi, uydurulmadi:
  duragan esigi 2.0 (olculen 0.914 ile 3.551 arasinda, duragan tarafa yakin)
  WARN 1.5 sn = profil `shot_min_sn` · FAIL 3.0 sn = iki kati

⚠ HAREKETLI VIDEO B-ROLL: I-16'daki BLOKE AYNEN GECERLI. Guvenli fixture
havuzunda gercek video adayi yok; depodaki `.mp4` dosyalari bu projenin
KENDI render ciktilaridir. Sahte B-roll KULLANILMAZ.

⚠ SAHTE ESIK DUSURME YOK. Medya benzerlik esigi (0.86) I-14'ten degismedi.

⚠ SAHTE VIDEO URETILMEZ. Gercek zincir:
  edit_kopru.plan_kur -> editor.plan.uret -> adapter.donustur ->
  remotion_v2 dogrula/props_hazirla/render -> Remotion `VidrushEditorV2`.
...
Cikti:
    outputs/sample/editorv2_motion_i17.mp4 (+ 9+ kare ve JSON rapor)
"""
...
os.environ.setdefault("VIDRUSH_KOK", os.path.join(DEPO, "cikti", "_i17_kok"))

CIKTI_DIZIN = os.path.join(DEPO, "outputs", "sample")
VIDEO_ADI = "editorv2_motion_i17.mp4"
FIXTURE = os.path.join(DEPO, "app", "render-studio", "public", "editorv2",
                       "faz_e")
CALISMA = os.path.join(DEPO, "cikti", "_i17")
...
def main() -> int:
    print("=" * 72)
    print("FAZ I-17 — BELGESEL MOTION GRAMMAR + OPTIK DURAGANLIK")
    print("=" * 72)
    ...
    for t in kare_anlari:
        ad = f"i17_kare_{str(t).replace('.', '_')}s.png"
    ...
    with open(os.path.join(CIKTI_DIZIN, "motion_i17_rapor.json"), "w",
              encoding="utf-8") as f:
        json.dump(rapor, f, ensure_ascii=False, indent=2)
    print(f"\n      rapor : outputs/sample/motion_i17_rapor.json")
